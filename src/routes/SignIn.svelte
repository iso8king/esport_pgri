<script>
  ...
  let captureInstruction = "";

  async function captureMultipleFrames(count = 6, intervalMs = 250) {
    const frames = [];
    for (let i = 0; i < count; i++) {
      if (i === Math.floor(count / 3)) {
        captureInstruction = "Kedipkan Matamu!";
      }
      if (i === Math.floor((count * 2) / 3)) {
        captureInstruction = "";
      }
  ...
  async function handleFaceLogin() {
    ...
    capturing = true;
    try {
      // 6 frame, jeda 250ms = total capture ~1.5 detik
      const frames = await captureMultipleFrames(6, 250);
...
<div
  class="min-h-screen bg-gray-700 flex items-center justify-center px-4 py-8 relative"
>
  <img
    src="/bg-login.jpg"
    alt="background"
  ...
  <div class="w-full max-w-md relative z-10">
    <div
      class="border border-gray-200 bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl overflow-hidden"
    >
      ...
      {#if activeTab === "password"}
        ...
        >
          ...
          <div class="pt-1 flex flex-col gap-4">
            <button
              type="submit"
              class="w-full bg-[#0b5ba2] hover:bg-[#0b4c8d] text-white font-bold py-3 rounded-2xl transition-colors tracking-wider"
            >
              SIGN IN
            </button>
      ...
      {:else}
        <div class="p-6 pt-4 space-y-4">
          <div
            class="relative rounded-2xl overflow-hidden bg-gray-900 aspect-square max-w-sm mx-auto"
          >
            ...
            <div
              class="absolute inset-6 border-2 border-white/40 rounded-2xl pointer-events-none"
            ></div>
          </div>

          <p class="text-xs text-gray-500 text-center">
            Posisikan wajahmu di dalam kotak, lalu <strong
              >berkedip normal</strong
            > saat proses verifikasi berjalan.
          </p>
